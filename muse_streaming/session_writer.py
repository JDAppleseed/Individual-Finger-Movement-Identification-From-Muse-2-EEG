...
def prepare_session_paths(
    *,
    ...
    if resume and session_dir.exists():
        return _build_session_paths(session_dir, session_id), True, "resume"
    unique_session_id = _ensure_unique_session_id(output_root, subject_id, session_id)
    if unique_session_id != session_id:
        session_dir = output_root / f"{subject_id}_{unique_session_id}"
        reason = f"new_session_collision({session_id})"
    else:
        reason = "new_session"
    session_dir.mkdir(parents=True, exist_ok=True)
    return _build_session_paths(session_dir, unique_session_id), False, reason


def _build_session_paths(session_dir: Path, session_id: str) -> SessionPaths:
...
class SessionWriter:
    def __init__(
        ...
        *,
        ...
        self._paths, self.resumed, self.reason = prepare_session_paths(
