...
def test_live_infer_rejects_bad_actuation_and_postprocess_values():
    result = validate_step_settings(
        "infer",
        {
            "serial_baud": "fast",
            "force_no_serial": "yes",
            "serial_movement_warmup_enabled": "yes",
            "serial_write_timeout_s": 0.0,
            "serial_max_hz": 0.0,
            "serial_settle_s": -1.0,
    ...
    assert any("force_no_serial must be a boolean." in err for err in result.errors)
    assert any(
        "serial_movement_warmup_enabled must be a boolean." in err
        for err in result.errors
    )
    assert any("serial_write_timeout_s must be > 0." in err for err in result.errors)
    assert any("serial_max_hz must be > 0." in err for err in result.errors)
    assert any("serial_settle_s must be >= 0.0." in err for err in result.errors)
    assert any(
