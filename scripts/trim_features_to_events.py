...
def main() -> int:
    ...
    p.add_argument("--features", required=True, type=str, help="Features CSV (eeg_features.csv)")
    p.add_argument("--events", required=True, type=str, help="Events CSV (events.csv)")
    p.add_argument("--margin-s", type=float, default=0.0, help="Seconds to expand trim window on both sides")
    p.add_argument("--inplace", action="store_true", help="Overwrite features file (creates .bak backup)")
    p.add_argument("--out", type=str, default="", help="Optional output path (ignored if --inplace)")
    args = p.parse_args()
    ...
    valid = onset.notna()
    if valid.sum() == 0:
        raise SystemExit("ERROR: events file has no valid onset_s values to trim against")
    ...
    if t_end <= t_start:
        raise SystemExit(f"ERROR: computed trim window invalid: start={t_start:.3f}, end={t_end:.3f}")
    ...
    if args.inplace:
        ...
        out_path = Path(args.out) if args.out else features_path.with_name(features_path.stem + "_TRIMMED.csv")
        df_out.to_csv(out_path, index=False)
        print("✅ Trim complete.")
        print(f"   Output: {out_path}")

    print(f"   Window: [{t_start:.3f}, {t_end:.3f}] seconds (margin={float(args.margin_s):.3f}s)")
    print(f"   Rows:   {n_before} -> {n_after}  (dropped {n_before - n_after})")

    # Quick sanity info
    if n_after == 0:
        print("⚠️ WARNING: No feature rows remain after trimming. Check time bases / margin.", file=sys.stderr)
