...
def _write_tables(runs: List[RunMetrics], demos: List[SubjectDemographics], session_meta: Dict[str, Any]) -> None:
    ...
    demo_lines.append("\\caption{Subject demographics (as available in \\texttt{subject.json}).}\n")
    ...
    dur_lines.append("\\begin{tabular}{lrrrr}\n\\toprule\n")
    ...
    for s in session_meta.get("sessions", []):
        ...
        dur_lines.append(
            f"{_latex_escape(str(s.get('session_id')))} & {int(recv or 0)} & {int(written or 0)} & {drop_str} & {dur_str} \\\\\n"
        )
