...
def _write_tables(runs: List[RunMetrics], demos: List[SubjectDemographics], session_meta: Dict[str, Any]) -> None:
    ...
    dur_lines.append("\\caption{Representative dataset provenance. The featured 2-M16 run is trained on a filtered derived dataset assembled from multiple recorded sessions.}\n")
    ...
    for r in ([featured] if featured is not None else []):
        ...
        if r.fig_loss_curve:
            ...
            lines.append(
                ...
                "only from recorded per-epoch history.}\n"
            )
