...
from demo_backend.nn_vis.extract import extract_activations, extract_architecture_manifest, extract_weights, load_model_and_weights, pack_tensor
...
def _resolve_source(path_str: str) -> Path:
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Source must be within repository root")
...
def _load_window_from_npz(npz_path: Path, index: int) -> tuple[np.ndarray, Optional[float], int]:
    data = np.load(npz_path, allow_pickle=True)
    if "X" in data:
        X = data["X"]
    ...
    if "test_indices" in data:
        ...
        if not fallback:
            locations = [str(ROOT), str(ROOT / "data" / "processed"), str(ROOT / "reports")]
            raise HTTPException(
                status_code=404,
...
async def nnvis_weights(
    ...
    model = _get_model()
    logger.info("nnvis_weights served quantize=%s downsample=%s topk=%s", quantize, downsample, topk)
    return extract_weights(model, quantize=bool(quantize), downsample=downsample, topk=topk)
...
async def nnvis_timeline_weights(file: str):
    ...
    if "weights" not in payload:
        raise HTTPException(status_code=400, detail="Timeline weights file missing 'weights'")
