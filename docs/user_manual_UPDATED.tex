...
\item running the desktop UI live workflow using LSL streams,
...
\item \textbf{Live desktop UI} streams LSL EEG via the GUI, runs inference, applies postprocessing, and surfaces decisions in the operator window.
\end{enumerate}
...
\item \textbf{\texttt{utils/inference.py}}: inference engine and confidence gating helpers.
\item \textbf{\texttt{utils/postprocess.py}}: stateful postprocessing for live stability.
\item \textbf{\texttt{eeglab\_wrapper\_ui.py}}: desktop UI entrypoint for live workflow.
\end{enumerate}
...
\section{Live desktop UI}
\subsection{Components}
\begin{itemize}[leftmargin=*]
\item \textbf{Desktop UI}: the \texttt{eeglab\_wrapper\_ui.py} application that runs streaming, inference, and operator controls.
...
\section{Desktop UI workflow (operator checklist)}
\begin{enumerate}[leftmargin=*]
\item Launch the UI: \texttt{python eeglab\_wrapper\_ui.py}
\item Click \textbf{Connect Muse 2} to start the internal BLE $\rightarrow$ LSL streamer.
\item Click \textbf{Start Recording} to launch \texttt{1\_stream\_and\_record.py} for live capture/inference.
\item Follow prompts for any channel/label warnings or hard stops.
\end{enumerate}
