...
def validate_cached_predictions_with_dataset_info(
    *,
    ...
    cache_subject_id = "" if cache_filters.get("subject_id") is None else str(cache_filters.get("subject_id"))
    current_subject_id = "" if current_filters.get("subject_id") is None else str(current_filters.get("subject_id"))
    ...
    if dataset_info_cache.get("experiment_hash") != dataset_info_current.get("experiment_hash"):
    ...
    if spotcheck_k > 0 and len(test_idx) > 0:
        ...
        for pos in positions:
            ...
            if y_action_test[pos] != y_action_current[idx] or y_finger_test[pos] != y_finger_current[idx]:
